...
export const Masonry: React.FC<MasonryProps> = ({
  ...
  const columns = columnsCount || responsiveColumns;
  ...
  useEffect(() => {
    const imageUrls = items.map(i => i.img).filter((url): url is string => Boolean(url));
  ...
  return (
    <div ref={containerRef} className={`masonry-list list ${className}`} style={{ height: totalHeight ? `${totalHeight}px` : 'auto' }}>
      {grid.map((item, index) => {
        return (
          <div
            key={item.id}
            data-key={item.id}
            className="masonry-item-wrapper item-wrapper"
            onClick={() => {
          ...
          >
            {renderItem ? (
              ...
              <div className="item-img" style={{ backgroundImage: item.img ? `url(${item.img})` : undefined }}>
                {colorShiftOnHover && (
                  <div
