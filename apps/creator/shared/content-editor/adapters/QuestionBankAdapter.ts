...
export class QuestionBankAdapter implements ContentDataAdapter {
  terminology = {
    root: "Question Bank",
    container: "Topic", // We'll hide containers in the UI, but this is required
    leafDocument: "Section",
    leafQuiz: "Quiz",
  };
  ...
  async loadContent(id: string): Promise<{ meta: ContentMeta; containers: ContainerNode[] }> {
    ...
    // For Question Bank, we only care about one root module to hold the sections
    let rootModule = course.modules[0];
    if (!rootModule) {
      rootModule = await api.post<ModuleResponse>(`/api/courses/${id}/modules`, { title: "Root Topic" });
    }
    ...
    return {
      ...
      containers: [{
        id: rootModule.id,
        title: rootModule.title,
        position: rootModule.position,
        expanded: true,
        leaves: [
          ...rootModule.lessons.map((l) => ({ ...l, type: "document" as const })),
        ],
      }],
    };
  }
