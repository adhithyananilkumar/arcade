...
const CursorGrid = ({
  ...
  useEffect(() => {
    ...
    let eventTarget = (eventTargetRef && eventTargetRef.current) || container.parentElement || container;
    if (eventTarget && typeof window !== 'undefined') {
      try {
        const style = window.getComputedStyle(eventTarget);
        if (style.pointerEvents === 'none') {
          let ancestor = eventTarget.parentElement;
          while (ancestor && ancestor !== document.body) {
            if (window.getComputedStyle(ancestor).pointerEvents !== 'none') {
        ...
        // Fallback to container if style checks fail
      }
    }
