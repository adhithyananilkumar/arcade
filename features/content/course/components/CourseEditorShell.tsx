...
import { QuizEditor, QuestionBankPanel } from "@/features/assessment";
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuContent,
...
import { publishService } from "@/features/learning/delivery/api/publish";
...
export function CourseEditorShell({ courseId: initialCourseId }: CourseEditorShellProps) {
  ...
  // ── Publish (D4 publishing domain) ────────────────────────────────────────
  // Cuts an immutable snapshot learners see at /published, distinct from this live working copy.
  ...
  const handlePublish = useCallback(async () => {
    ...
    if (editorRef.current) {
      try {
        ...
        // best-effort — proceed with publish regardless
      }
    }
    try {
      const result = await publishService.publish(courseId);
      setLastPublishedVersion(result.versionNumber);
    } catch (e) {
  ...
  // ── Preview (learner view of the live working copy) ───────────────────────
  // Opens the learner-facing renderer (mode="preview") in a new tab. Fire-and-forget flush
  // persists the latest edits first; window.open stays in the click gesture so it isn't blocked.

  const openPreview = () => {
    if (!courseId) return;
    editorRef.current?.flush().catch(() => {});
    window.open(`/dashboard/content/course/${courseId}/preview`, "_blank", "noopener");
  };
        ...
        <div className="mx-auto grid max-w-[1200px] grid-cols-[1fr_auto_1fr] items-center gap-2 px-4 py-1.5 sm:px-6">
          ...
          <div className="flex flex-shrink-0 items-center justify-self-end gap-1.5">
            ...
            <button
              ...
              onClick={openPreview}
              disabled={!courseId}
              title="Preview this course the way a learner sees it"
              className="inline-flex flex-shrink-0 items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-60"
            >
              <Eye size={15} />
              Preview
            </button>
